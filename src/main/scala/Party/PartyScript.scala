...
def compare(o: Iterable[cmp], n: Iterable[cmp]): Array[(String, Long, String)] = {
  ...
  val ecId = cOld(0)._1
    ...
    return cNew.map(x => (x._1,scala.math.abs(new scala.collection.immutable.StringOps(x._1).hashCode()).toLong, x._3)) //... else generate a new ECID
  }
}
//{
...
def hasNoSetRelation(o: Iterable[cmp], n: Iterable[cmp]): Boolean = { // do not compare clusters that have no set relations
  ...
  val cNew_filtered = cNew.filter(x => x._1 == -1) // identify pairs with no previous ECID
